...
public class KafkaTopicConfig {
	...
	public NewTopic paymentProcessedDLQ() {
		return TopicBuilder.name("payment-processed-dql").partitions(1).replicas(1).build();
	}
	
	@Bean
	public NewTopic stockFailed() {
		return TopicBuilder.name("stock-failed").partitions(1).replicas(1).build();
	}
	
	@Bean
	public NewTopic notificationRequest() {
		return TopicBuilder.name("notification-request").partitions(1).replicas(1).build();
	}
}